...
import { SMA200CrossOverStrategy } from "./SMA200CrossOverStrategy";
import { runStrategy } from "./strategyUtils";

/**
 * Call context:
 *   - Must be called in context of this folder because of the filePath context.
 *   - Cd from root and then run file: > cd src/strategies/ && npx ts-node runSMA200CrossOverStrategy.ts
 *   - Run from this folder context: > npx ts-node runSMA200CrossOverStrategy.ts
 *
 * Prepare 2 things:
...
(async function () {
    ...
    const buySignal = runStrategy(tulipDataStructure, SMA200CrossOverStrategy);
